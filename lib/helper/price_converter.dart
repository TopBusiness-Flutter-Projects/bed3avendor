...
class PriceConverter {
  static String convertPrice(BuildContext context, double? price,
    ...
    bool _singleCurrency =
        _splashProvider.configModel!.currencyModel == 'single_currency';

    return '${_splashProvider.myCurrency!.symbol}${(_singleCurrency ? price : price! * _splashProvider.myCurrency!.exchangeRate! * (1 / _splashProvider.usdCurrency!.exchangeRate!))!.toStringAsFixed(_splashProvider.configModel!.decimalPointSettings!).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}';
  }
  ...
  static String convertPriceWithoutSymbol(BuildContext context, double? price,
    ...
    return '${(_singleCurrency ? price : price! * _splashProvider.myCurrency!.exchangeRate! * (1 / _splashProvider.usdCurrency!.exchangeRate!))!.toStringAsFixed(_splashProvider.configModel!.decimalPointSettings!)}';
  }
  ...
  static double convertAmount(double amount, BuildContext context) {
    final _splashProvider = Provider.of<SplashProvider>(context, listen: false);
    return double.parse(
        '${(amount * _splashProvider.myCurrency!.exchangeRate! * (1 / _splashProvider.usdCurrency!.exchangeRate!)).toStringAsFixed(_splashProvider.configModel!.decimalPointSettings!)}');
  }
