...
import 'package:bed3avendor/data/model/response/add_product_model.dart';
import 'package:bed3avendor/data/model/response/attr.dart';
import 'package:bed3avendor/data/model/response/attribute_model.dart';
import 'package:bed3avendor/data/model/response/base/api_response.dart';
import 'package:bed3avendor/data/model/response/base/error_response.dart';
import 'package:bed3avendor/data/model/response/brand_model.dart';
import 'package:bed3avendor/data/model/response/category_model.dart';
import 'package:bed3avendor/data/model/response/config_model.dart';
import 'package:bed3avendor/data/model/response/edt_product_model.dart';
import 'package:bed3avendor/data/model/response/image_model.dart';
import 'package:bed3avendor/data/model/response/product_model.dart';
import 'package:bed3avendor/data/model/response/variant_type_model.dart';
...
class SellerProvider extends ChangeNotifier {
  ...
  void getAttributeList(
    ...
    if (response.response != null && response.response!.statusCode == 200) {
      ...
      _attributeList!.add(AttributeModel(
          attribute: Attr(id: 0, name: 'ColorX'),
          active: false,
      ...
      response.response!.data.forEach((attribute) {
        if (product != null && product.attributes != null) {
          ...
          print(
              '--------${Attr.fromJson(attribute).id}/$_active/${product.attributes}');
          List<String> _options = [];
          if (_active) {
            _options.addAll(product
                .choiceOptions![
  ...
  Future<void> getSubSubCategoryList(
  ...
  Future addProductImage(BuildContext context, ImageModel imageForUpload,
    ...
    if (response.response != null && response.response!.statusCode == 200) {
      ...
      if (type == 'product') {
        ...
        if (attributeList![0].active) {
          colorImageObject.add(ColorImage(
              color:
                  map['color_image'] != null ? map['color_image']['color'] : '',
              imageName: name));
        } else {
          colorImageObject = [];
        }
      ...
      ApiChecker.checkApi(context, response);
      showCustomSnackBar(
          getTranslated('image_upload_failed', context), context);
    }
    notifyListeners();
  }
  ...
  Future<void> addProduct(
      ...
      _attributeList!.forEach((attributeModel) {
        if (attributeModel.active) {
          ...
          _fields.addAll(<String, dynamic>{
            'choice_options_${attributeModel.attribute.id}': _variantString
          });
        }
      });
      _fields.addAll(<String, dynamic>{
        'choice_attributes': _idList,
        'choice_no': _idList,
        'choice': _nameList
      });
    ...
    if (response.response != null && response.response!.statusCode == 200) {
      ...
      showCustomSnackBar(
          isAdd
              ? getTranslated('product_added_successfully', context)
              : getTranslated('product_updated_successfully', context),
          context,
          isError: false);
  ...
  Future<void> deleteProduct(BuildContext context, int? productID) async {
    ...
    if (response.response!.statusCode == 200) {
      ...
      showCustomSnackBar(
          getTranslated('product_deleted_successfully', context), context,
          isError: false);
    } else {
      ApiChecker.checkApi(context, response);
    }
    ...
    for (int i = 0; i < _length; i++) {
      ...
      if (product != null) {
        ...
        _variantTypeList.add(VariantTypeModel(
          ...
          controller: TextEditingController(
              text: _price! > 0
                  ? PriceConverter.convertPriceWithoutSymbol(context, _price)
                  : ''),
          node: FocusNode(),
          qtyController: TextEditingController(text: _quantity.toString()),
          qtyNode: FocusNode(),
        ));
      } else {
        _variantTypeList.add(VariantTypeModel(
      ...
      for (int j = 0; j < _mainList.length; j++) {
        if (_indexList[_indexList.length - (1 + j)] <
            _mainList[_mainList.length - (1 + j)].length - 1) {
          _indexList[_indexList.length - (1 + j)] =
              _indexList[_indexList.length - (1 + j)] + 1;
          break;
        } else {
  ...
  Future<void> updateProductQuantity(BuildContext context, int productId,
    ...
    ApiResponse apiResponse = await shopRepo!
        .updateProductQuantity(productId, currentStock, variations);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      Navigator.pop(context);
      showCustomSnackBar(
          getTranslated('quantity_updated_successfully', context), context,
          isError: false);
  ...
  Future<ApiResponse> uploadReadyAfterSellDigitalProduct(BuildContext context,
    ...
    if (response.response!.statusCode == 200) {
      _isLoading = false;
      showCustomSnackBar(
          getTranslated("digital_product_uploaded_successfully", context),
          context,
          isError: false);
    } else {
      _isLoading = false;
