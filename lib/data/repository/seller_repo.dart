...
class SellerRepo {
  ...
  Future<ApiResponse> uploadDigitalProduct(File? filePath, String token) async {
    http.MultipartRequest request = http.MultipartRequest(
        'POST',
        Uri.parse(
            '${AppConstants.BASE_URL}${AppConstants.DIGITAL_PRODUCT_UPLOAD}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    if (filePath != null) {
      Uint8List _list = await filePath.readAsBytes();
      var part = http.MultipartFile(
          'digital_file_ready', filePath.readAsBytes().asStream(), _list.length,
  ...
  Future<ApiResponse> uploadAfterSellDigitalProduct(
    ...
    http.MultipartRequest request = http.MultipartRequest(
        'POST',
        Uri.parse(
            '${AppConstants.BASE_URL}${AppConstants.DIGITAL_PRODUCT_UPLOAD_AFTER_SELL}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    print('Here is ===>$filePath');
    if (filePath != null) {
      Uint8List _list = await filePath.readAsBytes();
      var part = http.MultipartFile('digital_file_after_sell',
          filePath.readAsBytes().asStream(), _list.length,
          filename: basename(filePath.path));
      request.files.add(part);
    }

    Map<String, String> _fields = Map();
    _fields.addAll(<String, String>{'order_id': orderId, '_method': 'put'});
